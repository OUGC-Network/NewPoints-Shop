...
function newpoints_shop_install()
{
    ...
    $db->write_query(
        'ALTER TABLE `' . TABLE_PREFIX . 'newpoints_grouprules` ADD `items_rate` DECIMAL(6,3) NOT NULL default 1.000;'
    );

    // add settings
    newpoints_add_setting(
        'newpoints_shop_sendable',
        'newpoints_shop',
        'Send items',
        'Allow users to send items to other users.',
        'yesno',
        '1',
        1
    );
    newpoints_add_setting(
        'newpoints_shop_sellable',
        'newpoints_shop',
        'Sell items',
    ...
    newpoints_add_setting(
        'newpoints_shop_lastpurchases',
        'newpoints_shop',
        'Last Purchases',
        'Number of last purchases to show in statistics.',
        'text',
        '10',
        3
    );
    newpoints_add_setting(
        'newpoints_shop_percent',
        'newpoints_shop',
        'Sell Percentage',
        'The (discounted) rate at which items can be sold for.',
        'text',
        '0.75',
        4
    );
    newpoints_add_setting(
        'newpoints_shop_viewothers',
        'newpoints_shop',
        'Can View Others\' inventories',
        'Allow users to view other users\' inventories. Note, admins always are able to view other users\' inventories.',
        'yesno',
        '1',
        5
    );
    newpoints_add_setting(
        'newpoints_shop_itemsprofile',
        'newpoints_shop',
        'Items on profile',
        'Number of items to show in profile page. Set to 0 to disable this feature.',
        'text',
        '5',
        6
    );
    newpoints_add_setting(
        'newpoints_shop_itemspostbit',
        'newpoints_shop',
        'Items on postbit',
        'Number of items to show in postbit. Set to 0 to disable this feature.',
        'text',
        '5',
        7
    );
...
function newpoints_shop_uninstall()
{
    ...
    newpoints_remove_settings(
        "'newpoints_shop_sendable','newpoints_shop_sellable','newpoints_shop_lastpurchases','newpoints_shop_percent','newpoints_shop_viewothers','newpoints_shop_itemsprofile','newpoints_shop_itemspostbit'"
    );
    rebuild_settings();
    ...
    newpoints_remove_log(['shop_purchase', 'shop_send', 'shop_sell']);
}
...
'
    ...
    newpoints_add_template(
        'newpoints_shop_myitems_no_items',
        '
<tr>
...
'
    ...
    newpoints_find_replace_templatesets(
        ...
        '#' . preg_quote('width="60%">') . '#',
        'width="60%">{$newpoints_shop_lastpurchases}'
    );
    newpoints_find_replace_templatesets(
        'newpoints_postbit',
        '#' . preg_quote('{$donate}') . '#',
        '{$donate}{$post[\'newpoints_shop_items\']}'
    );
    ...
    find_replace_templatesets(
        ...
        '#' . preg_quote('</ul>') . '#',
        '<li><a href="{$mybb->settings[\'bburl\']}/{$config[\'admin_dir\']}/index.php?module=newpoints-shop&amp;action=inventory&amp;uid={$uid}">{$lang->newpoints_shop_edit_inventory}</a></li></ul>'
    );
}
...
function newpoints_shop_deactivate()
{
    ...
    find_replace_templatesets(
        'member_profile_adminoptions',
        '#' . preg_quote(
            '<li><a href="{$mybb->settings[\'bburl\']}/{$config[\'admin_dir\']}/index.php?module=newpoints-shop&amp;action=inventory&amp;uid={$uid}">{$lang->newpoints_shop_edit_inventory}</a></li>'
        ) . '#',
        '',
        0
    );
        ...
        switch ($mybb->input['shop_action']) {
            case 'buy':
                ...
                if (!empty($errors)) {
                    ...
                    $db->update_query(
                        'users',
                        ['newpoints_items' => serialize($myitems)],
                        'uid=\'' . $mybb->user['uid'] . '\''
                    );
                    ...
                    if (!empty($item['pm']) || $mybb->settings['newpoints_shop_pm_default'] != '') {
                        ...
                        if ($item['pm'] == '' && $mybb->settings['newpoints_shop_pm_default'] != '') {
                            $item['pm'] = str_replace(['{itemname}', '{itemid}'],
                                [$item['name'], $item['iid']],
                                $mybb->settings['newpoints_shop_pm_default']);
                        }
                    ...
                    if (!empty($item['pmadmin']) || $mybb->settings['newpoints_shop_pmadmins'] != '') {
                        ...
                        if ($item['pmadmin'] == '' && $mybb->settings['newpoints_shop_pm_default'] != '') {
                            $item['pmadmin'] = str_replace(['{itemname}', '{itemid}'],
                                [$item['name'], $item['iid']],
                                $mybb->settings['newpoints_shop_pmadmin_default']);
                        }
                        ...
                        newpoints_send_pm(
                            [
                                ...
                                'touid' => [explode(',', $mybb->settings['newpoints_shop_pmadmins'])],
                                'receivepms' => 1
                            ],
                            $mybb->user['uid']
                        );
            ...
            case 'do_send':
                ...
                if (!($user = newpoints_getuser_byname($username))) {
                    ...
                    $db->update_query(
                        'users',
                        ['newpoints_items' => serialize($useritems)],
                        'uid=\'' . $user['uid'] . '\''
                    );
            ...
            case 'sell':
                ...
                $data = "<td class=\"trow1\" width=\"100%\">" . $lang->sprintf(
                        ...
                        htmlspecialchars_uni($item['name']),
                        newpoints_format_points(floatval($item['price']) * $mybb->settings['newpoints_shop_percent'])
                    ) . '</td>';
            ...
            case 'do_sell':
                ...
                newpoints_addpoints(
                    $mybb->user['uid'],
                    floatval($item['price']) * $mybb->settings['newpoints_shop_percent']
                );
    ...
    if (empty($memprofile['newpoints_items'])) {
        ...
        if (!empty($items)) {
            ...
            $query = $db->simple_select(
                ...
                ['limit' => intval($mybb->settings['newpoints_shop_itemsprofile'])]
            );
            while ($item = $db->fetch_array($query)) {
                if ($item['icon'] != '') {
...
function newpoints_shop_stats()
{
    ...
    $query = $db->simple_select(
        ...
        [
            'order_by' => 'date',
            'order_dir' => 'DESC',
            'limit' => intval($mybb->settings['newpoints_shop_lastpurchases'])
        ]
    );
...
function newpoints_shop_admin()
{
    ...
    if ($run_module == 'newpoints' && $action_file == 'newpoints_shop') {
        if ($mybb->request_method == 'post') {
            switch ($mybb->input['action']) {
                ...
                case 'do_editcat':
                    ...
                    if ($cid <= 0 || (!($cat = $db->fetch_array(
                            $db->simple_select('newpoints_shop_categories', '*', "cid = $cid")
                        )))) {
                ...
                case 'do_edititem':
                    ...
                    if ($iid <= 0 || (!($item = $db->fetch_array(
                            $db->simple_select('newpoints_shop_items', '*', "iid = $iid")
                        )))) {
        ...
        if ($mybb->input['action'] == 'do_deletecat') {
            ...
            if ($mybb->request_method == 'post') {
                if ($cid <= 0 || (!($cat = $db->fetch_array(
                        $db->simple_select('newpoints_shop_categories', 'cid', "cid = $cid")
                    )))) {
                    newpoints_shop_messageredirect($lang->newpoints_shop_invalid_cat, 1);
                }
                ...
                $form = new Form(
                    "index.php?module=newpoints-shop&amp;action=do_deletecat&amp;cid={$mybb->input['cid']}&amp;my_post_key={$mybb->post_code}",
                    'post'
                );
            ...
            if ($mybb->request_method == 'post') {
                if ($iid <= 0 || (!($item = $db->fetch_array(
                        $db->simple_select('newpoints_shop_items', 'cid', "iid = $iid")
                    )))) {
                ...
                $form = new Form(
                    "index.php?module=newpoints-shop&amp;action=do_deleteitem&amp;iid={$mybb->input['iid']}&amp;my_post_key={$mybb->post_code}",
                    'post'
                );
                echo "<div class=\"confirm_action\">\n";
            ...
            if ($mybb->request_method == 'post') {
                ...
                unset($inventory[$key]);
                sort($inventory);
                $db->update_query('users', ['newpoints_items' => serialize($inventory)], 'uid=\'' . $uid . '\'');
                ...
                $form = new Form(
                    "index.php?module=newpoints-shop&amp;action=remove&amp;iid={$mybb->input['iid']}&amp;uid={$mybb->input['uid']}&amp;my_post_key={$mybb->post_code}",
                    'post'
                );
                echo "<div class=\"confirm_action\">\n";
        ...
        if (!$mybb->input['action'] || $mybb->input['action'] == 'categories' || $mybb->input['action'] == 'inventory' || $mybb->input['action'] == 'addcat' || $mybb->input['action'] == 'editcat') {
            ...
            if (!$mybb->input['action'] || $mybb->input['action'] == 'categories' || $mybb->input['action'] == 'addcat' || $mybb->input['action'] == 'editcat') {
                ...
                $sub_tabs['newpoints_shop_categories_delete'] = [
        ...
        if ($mybb->input['action'] == 'items' || $mybb->input['action'] == 'additem' || $mybb->input['action'] == 'edititem') {
            ...
            if ($mybb->input['action'] == 'items' || $mybb->input['action'] == 'additem' || $mybb->input['action'] == 'edititem') {
                ...
                $sub_tabs['newpoints_shop_items_delete'] = [
        ...
        if (!$mybb->input['action'] || $mybb->input['action'] == 'categories') {
            ...
            while ($cat = $db->fetch_array($query)) {
                ...
                $table->construct_cell(intval($cat['items']), ['class' => 'align_center']);
                $table->construct_cell(intval($cat['disporder']), ['class' => 'align_center']);
            ...
            $form_container->output_row(
                $lang->newpoints_shop_addedit_cat_visible,
                $lang->newpoints_shop_addedit_cat_visible_desc,
                $form->generate_yes_no_radio('visible', intval($cat['visible'])),
                'visible'
            );
            $form_container->output_row(
                $lang->newpoints_shop_addedit_cat_icon,
            ...
            $form_container->output_row(
                $lang->newpoints_shop_addedit_cat_disporder,
                $lang->newpoints_shop_addedit_cat_disporder_desc,
                $form->generate_text_box('disporder', intval($cat['disporder']), ['id' => 'disporder']
                ),
                'disporder'
            );
            $form_container->output_row(
                $lang->newpoints_shop_addedit_cat_expanded,
                $lang->newpoints_shop_addedit_cat_expanded_desc,
                $form->generate_yes_no_radio('expanded', intval($cat['expanded'])),
                'expanded'
            );
            ...
            $form->output_submit_wrapper($buttons);
            $form->end();
        } elseif ($mybb->input['action'] == 'items') {
            ...
            while ($item = $db->fetch_array($query)) {
                ...
                $table->construct_cell(newpoints_format_points($item['price']), ['class' => 'align_center']);
                $table->construct_cell(intval($item['disporder']), ['class' => 'align_center']);
            ...
            $args = $plugins->run_hooks('newpoints_shop_row', $args);
            ...
            $form_container->output_row(
                $lang->newpoints_shop_addedit_item_price,
                $lang->newpoints_shop_addedit_item_price_desc,
                $form->generate_text_box('price', floatval($item['price']), ['id' => 'price']
                ),
                'price'
            );
            $form_container->output_row(
            ...
            $form_container->output_row(
                $lang->newpoints_shop_addedit_item_disporder,
                $lang->newpoints_shop_addedit_item_disporder_desc,
                $form->generate_text_box('disporder', intval($item['disporder']), ['id' => 'disporder']
                ),
                'disporder'
            );
            $form_container->output_row(
                $lang->newpoints_shop_addedit_item_stock,
                $lang->newpoints_shop_addedit_item_stock_desc,
                $form->generate_text_box('stock', intval($item['stock']), ['id' => 'stock']
                ),
                'stock'
            );
            $form_container->output_row(
                $lang->newpoints_shop_addedit_item_infinite,
                $lang->newpoints_shop_addedit_item_infinite_desc,
                $form->generate_yes_no_radio('infinite', intval($item['infinite'])),
                'infinite'
            );
            $form_container->output_row(
                $lang->newpoints_shop_addedit_item_limit,
                $lang->newpoints_shop_addedit_item_limit_desc,
                $form->generate_text_box('limit', intval($item['limit']), ['id' => 'limit']
                ),
                'limit'
            );
            $form_container->output_row(
                $lang->newpoints_shop_addedit_item_visible,
                $lang->newpoints_shop_addedit_item_visible_desc,
                $form->generate_yes_no_radio('visible', intval($item['visible'])),
                'visible'
            );
            $form_container->output_row(
                $lang->newpoints_shop_addedit_item_sendable,
                $lang->newpoints_shop_addedit_item_sendable_desc,
                $form->generate_yes_no_radio('sendable', intval($item['sendable'])),
                'sendable'
            );
            $form_container->output_row(
                $lang->newpoints_shop_addedit_item_sellable,
                $lang->newpoints_shop_addedit_item_sellable_desc,
                $form->generate_yes_no_radio('sellable', intval($item['sellable'])),
                'sellable'
            );
            $form_container->output_row(
                $lang->newpoints_shop_addedit_item_pm,
            ...
            $form_container->output_row(
                $lang->newpoints_shop_addedit_item_category . '<em>*</em>',
                $lang->newpoints_shop_addedit_item_category_desc,
                $form->generate_select_box('cid', $categories, intval($item['cid']), ['id' => 'cid']
                ),
                'cid'
            );
            ...
            $form->output_submit_wrapper($buttons);
            $form->end();
        } elseif ($mybb->input['action'] == 'inventory') {
...
function newpoints_shop_admin_stats()
{
    ...
    while ($stats = $db->fetch_array($query)) {
        ...
        $table->construct_cell(newpoints_format_points($data[1]), ['class' => 'align_center']);
        $table->construct_cell(
...
function newpoints_shop_admin_rule(&$form_container)
{
    ...
    if ($mybb->input['action'] == 'add') {
        $form_container->output_row(
            ...
            $form->generate_text_box('items_rate', 1, ['id' => 'items_rate']
            ),
            'items_rate'
        );
        ...
        $form_container->output_row(
            $lang->newpoints_shop_items_rate,
            $lang->newpoints_shop_items_rate_desc,
            $form->generate_text_box('items_rate', $rule['items_rate'], ['id' => 'items_rate']
            ),
            'items_rate'
        );
    }
